...
![클라우드플레어_7](./Django_gcp/클라우드플레어_활성메일.PNG)

<mark>클라우드 플레어에서 도메인이 활성화되면 위와같은 메일이온다.</mark>

메일을 받았으니, <u>클라우드플레어에 들어가 도메인이 활성화</u>되어있는지 재차 확인해본다.

![클라우드플레어_8](./Django_gcp/클라우드플레어_도메인활성.PNG)

위와같이 <u>자신이 등록한 도메인의 이름 아래에 활성됨</u>이라고 뜨면된다.


**이제 도메인도 활성되었으니 nginx에 도메인을 알려줘야한다.**



<mark>결과</mark>

![클라우드플레어_9](./Django_gcp/클라우드플레어_배포완료.PNG)


nginx와 uwsgi을 django프로젝트를 실행시키는 인스턴스가 시작되기만하면 자동으로 켜지게 등록해놨기때문에, 인스턴스를 켜면 내가 등록한 도메인으로 접속할수있다.

<mark>nginx에 도메인추가하기</mark>

nginx의 conf파일, 즉 위에서 만들었던 test.conf파일에 최종적으로 아래와같이 작성하였다.


    server {
        listen 80;
        server_name your_domian 해당인스턴스내부아이피호스트명 #내부아이피호스트명은 안써도됨.
        charset utf-8;
        client_max_body_size 128M;

        location / {
            uwsgi_pass  unix:///tmp/mysite(<프로젝트명).sock;
            include     uwsgi_params;
        }
    }


```*.your_hosts_name.net *.yor_domain.com;```

*.호트스명, *.도메인명으로쓰면 *가 와일드카드역할을하여 도메인명이나 호스트명앞에 어떤것이와도된다.

ex)www가들어오면 www.your_domain.com, www.your_hosts_name.net 이 되는것.

test.conf파일, 즉 nginx의 설정이 변경되었으니 반영해줘야한다.

    sudo cp -f /home/계정명/장고프로젝트폴더명/.config/nginx/test.conf /etc/nginx/sites-available/test.conf

    sudo ln -sf /etc/nginx/sites-available/test.conf /etc/nginx/sites-enabled/test.conf

이제 nginx의 설정파일을 nginx의 설정을 실제로 반영하는 폴더에 적용하였으니 <u>서비스를 재시작</u> 해주자.

    sudo systemctl daemon-reload && sudo systemctl restart nginx uwsgi


이렇게 설정해주면 위의 결과사진과같이 가비아에서 구매한 도메인이름으로 장고프로젝트가 배포되고있는것을 볼수있다.

※단, 클라우드플레어로 도메인을 연결하여 서비스를 실행하니 접속이 조금 느리다. 이유를 찾아보니 클라우드 플레어는 해외를 경유해서 접속하는경우가많기때문에 사용자입장에서 느리다고 느낄수있다고한다.

그래서 클라우드 플레어는 반드시 필요한 경우가아니면 사용을 고려해보는것도 나쁘지않을것같다. + 반드시 써야하는경우면 미국서버에 서버를 세팅해서 국내에서 서비스한다고도한다.


4. SSL인증서발급

SSL인증서 발급에는 Let`s Encrypt 를 이용하거나, 클라우드플레어 자체에서도 SSL인증서를 발급해준다고한다.

조금 찾아보니 둘 다 인증서를 발급해주는데, 이왕 클라우드플레어에 도메인을 연결해서 사용중이니, 클라우드 플레어에서 인증서를 발급해볼것이다.

클라우드 플레어에서 SSL인증서를 발급받는것은 간단하다.

![클라우드플레어_ssl](./Django_gcp/클라우드플레어_SSL인증서.PNG)

![클라우드플레어_ssl2](./Django_gcp/클라우드플레어_ssl활성.PNG)


SSL을 적용하려는 도메인을 대시보드에서 선택한후, SSL 메뉴를 클릭하고 가변, 전체 중 하나를 선택하면된다.

가변으로하면 nginx의 server부분에 

```listen 443 ssl your_domain...```

위의 코드를 작성해주지않아도 80번포트만 연결해줘도 자동으로 ssl 443포트로 연결이된다.

SSL메뉴부분에서 위와같이 설정해주고 <u>DNS메뉴로 이동</u>하여 도메인 레코드를 추가해줬던부분에서 SSL을 적용할 도메인에 프록시상태를 바꿔준다.

![클라우드플레어_ssl3](./Django_gcp/클라우드플레어_ssl프록시.PNG)

<u>프록시상태를 DNS onyl -> 프록싱됨</u> 으로 변경해준다.

이렇게 설정해주면 사용자가 https://your_domian.com/ 와같이 https로 페이지를 요청하면 나의 장고프로젝트 웹페이지에 SSL 인증서가 적용된것을 확인할수있다.

![클라우드플레어_ssl4](./Django_gcp/클라우드플레어_ssl확인.PNG)

▲ 빨간 네모가 ssl 인증서가 적용되고있다는 표시

<mark>하지만, http나 www로 들어오는 요청은 인증서가 적용되지않은 http 웹페이지로 반환된다.</mark>

이러한 요청들을 https로 받기위해 nginx의 conf파일을 변경해주던가 클라우드 플레어에서 규칙을 설정해줘야한다.


![클라우드플레어_ssl5](./Django_gcp/클라우드플레어_규칙.PNG)


클라우드플레어 대시보드의 메뉴에서 규칙 -> Page Rules를 선택해서 **http를 https리다이렉트해주는 규칙을 설정해주자.**

기본적으로 3개를 준다고하는데 3개이상이면 nginx의 conf파일에서 리다이렉션해주는것으로 처리하는것도 나쁘지않을것같다.

이제 페이지 규칙을 추가해주자.


![클라우드플레어_ssl6](./Django_gcp/트클라우드플레어_리다이렉트설정.PNG)



    URL : 리다이렉트 규칙을 적용할 도메인

    EX) http://your_domain.com/*


이후 아래 설정 선택에서 전달 URL을 선택한후, 옆의 상태코드에서 301-영구 리디렉션을 선택하고, <u>대상 URL입력</u>에 바꿔줄 url을 입력한다.

    전달 URL: 위의 URL에서 바꿔줄 값
    EX) https://your_domain.com/*



위와같이 입력하고 Page Rule 저장을 누르면 성공적으로 추가되고, http로 요청을 보냈을시 https로 바뀌어서 접속이된다.

그러므로 **http로 요청해도 https로들어가기때문에 ssl인증서가 잘 적용이된다.**

SSL인증서가 잘 적용된 상태로 현재 배포하고있는 서비스의 현재위치 추적 기능과 현재위치 이동기능 등, 맵 관련해서 내 위치 추적 기능이 잘 작동하는것을 확인했다.

+)

<u>나는 카카오 지도 API를 이용하여</u> 주변 상점 위치를 제공하는 페이지를 구현하였는데, <u>위치이동</u>기능이 작동하지않았다.

그래서 API KEY의 문제인가하여 문제를 해결하던 도중, 카카오 API를 사용할때 API를 사용할 사이트를 카카오 API에 등록해줘야한다는것을 까먹고있었다는걸 기억해냈다.

[카카오지도API](https://apis.map.kakao.com/)

해당 링크에 들어가서 현재 프로젝트에 사용중인 앱 선택>
왼쪽 메뉴의 플랫폼 > 맨 아래의 Web부분에서 사이트 도메인에 <u>카카오 API를 사용하고있는 프로젝트의 도메인 or ip주소</u>를 넣고 저장.

![카카오도메인등록](./Django_gcp/카카오api_도메인등록.PNG)


위 사진과같은 부분에 카카오 api를 사용할 웹페이지의 도메인이나 ip주소를 입력해주면 위치이동기능이 제대로 작동하는것을 확인할수있다.
