...
## Private Repositories 만들고 접근하기

git에 파일을올릴때 ignoer파일을 이용하여 예외처리를 할수도있겠지만, 클라우드 인스턴스를 백업 개념으로 사용할때는 비공개 레포지토리에 올리는게 훨씬 편하다.

1. NEW 레포지토리를 생성하는 과정에서 레포지토리를 비공개로 설정하여 생성.


2. 내 계정 아이콘을 클릭하고 제일 하단의 Settings 클릭 > 좌측 제일 하단의 Developer settings 클릭

3. 아래 그림과같이 비공개 레포지토리에 접근할수있는 토큰 생성을하자. Generate new token 클릭

![git 토큰 생성](./git/git_토큰생성.PNG)

4. 패스워드를 입력하고 다음 화면과같이 only select repositories를 눌러 토큰을 적용할 레포지토리(비공개)를 골라준다. 복수선택도 가능하니 필요한만큼 지정가능.
이후, **Permissions에서 허락할 권한을 지정해주고** 생성하면 끝.

![git토큰권한주기](./git/git_토큰생성2.PNG)

5. 생성된 토큰을 어딘가에 잘 보관해두고 해당 레포지토리에 접근할때 password로 사용하면된다.